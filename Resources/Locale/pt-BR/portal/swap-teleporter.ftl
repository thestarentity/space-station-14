swap-teleporter-popup-link-create = Ligação quântica estabelecida!
swap-teleporter-popup-link-fail-already = A ligação quântica falhou! Já existe uma ligação presente no dispositivo.
swap-teleporter-popup-link-fail-already-other = Falha na ligação quântica! A ligação já está presente no dispositivo secundário.
swap-teleporter-popup-link-destroyed = Ligação quântica destruída!
swap-teleporter-popup-teleport-cancel-time = Ele ainda está recarregando!
swap-teleporter-popup-teleport-cancel-link = Ele não está vinculado a outro dispositivo!
swap-teleporter-popup-teleport-other = { CAPITALIZE(THE($entity)) } ativa, e você se encontra em outro lugar.
swap-teleporter-popup-teleport-fail = { CAPITALIZE(THE($entity)) } ativa e falha ao transportar você para qualquer lugar.
swap-teleporter-verb-destroy-link = Destrua o Link Quântico
swap-teleporter-examine-link-present = [color=forestgreen]Ele está ligado a outro dispositivo.[/color] Clique com o botão alternativo para quebrar o link quântico.
swap-teleporter-examine-link-absent = [color=yellow]Não está vinculado no momento.[/color] Use em outro dispositivo para estabelecer um link quântico.
swap-teleporter-examine-time-remaining =
    Tempo restante para recarregar: [color=purple]{ $second } segundo{ $second ->
        [one].
       *[other]s.
    }[/color]
