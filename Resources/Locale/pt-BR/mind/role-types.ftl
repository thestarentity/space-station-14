role-type-crew-aligned-name = Tripulante Alinhado
role-type-solo-antagonist-name = Antagonista Solitário
role-type-team-antagonist-name = Antagonista da Equipe
role-type-free-agent-name = Agente Livre
role-type-familiar-name = Familiar
role-type-silicon-name = Silício
role-type-silicon-antagonist-name = Silício Alterado
role-type-update-message = Seu cargo é [color={ $color }]{ $role }[/color]
# If you change a color here, you might want to also change it in role_types.yml
role-type-crew-aligned-color = Crew Aligned
role-type-crew-aligned-alternate-color = Verde
role-type-solo-antagonist-color = Vermelho
role-type-team-antagonist-color = Vermelho
role-type-free-agent-color = Amarelo
role-type-familiar-color = Azul
role-type-silicon-color = Azul
role-type-silicon-antagonist-color = Violeta
# Ideally, subtype names should be short
role-subtype-traitor = Traidor
role-subtype-thief = Ladrão
role-subtype-ninja = Ninja
role-subtype-nukie = Nukie
role-subtype-traitor-reinforcement = Reforço
role-subtype-revolutionary = Revolucionário
role-subtype-head-revolutionary = Revolutionário
role-subtype-initial-infected = Infestado
role-subtype-zombie = Zumbi
role-subtype-dragon = Dragão
role-subtype-survivor = Sobrevivente
role-subtype-subverted = Subvertido
role-subtype-paradox-clone = Paradox
role-subtype-wizard = Feiticeiro
role-subtype-xenoborg = Xenoborg
role-subtype-xenoborg-core = Núcleo Xenoborg
role-subtype-changeling = Mudança de forma
