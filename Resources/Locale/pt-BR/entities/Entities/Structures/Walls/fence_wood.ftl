ent-FenceWoodHighGate = portão de cerca de madeira
    .desc = Você tem alguma ideia do que o espera atrás dessas portas? Pode ser um banheiro ou uma luxuosa mansão. Mas você continua a amar seus garotos emo.

ent-FenceWoodSmallGate = portão de cerca de madeira
    .desc = Olhando para essa porta, uma imagem familiar surge na sua cabeça. Onde está o meu porquinho?

ent-BaseFenceWood =
    .desc = Uma cerca de madeira. Espero que haja o jardim da vovó atrás dela.

ent-BaseFenceWoodSmall =
    .desc = Uma cerca de madeira pequena. A melhor proteção para delimitar um território privado!
