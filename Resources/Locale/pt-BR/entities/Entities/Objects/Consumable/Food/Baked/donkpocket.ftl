ent-FoodDonkpocket = donk-pocket
    .desc = A escolha de comida para o traiçoeiro experiente.

ent-FoodDonkpocketWarm = donk-pocket quente
    .desc = O alimento aquecido de escolha para o traiçoeiro experiente.

ent-FoodDonkpocketDank = dank-pocket
    .desc = A escolha de comida para o botanista experiente.

ent-FoodDonkpocketDankWarm = dank-pocket quente
    .desc = A opção de alimento aquecido preferida pelo botanista experiente.

ent-FoodDonkpocketSpicy = spicy-pocket
    .desc = O lanche clássico, agora com um toque picante ativado pelo calor.

ent-FoodDonkpocketSpicyWarm = spicy-pocket quente
    .desc = O lanche clássico, agora talvez um pouco muito picante.

ent-FoodDonkpocketTeriyaki = teriyaki-pocket
    .desc = Uma versão asiática oriental do clássico lanche da estação.

ent-FoodDonkpocketTeriyakiWarm = teriyaki-pocket quente
    .desc = Uma versão asiática oriental do clássico lanche da estação, agora quente e fumegante.

ent-FoodDonkpocketPizza = pizza-pocket
    .desc = Delicioso, com queijo e surpreendentemente saciante.

ent-FoodDonkpocketPizzaWarm = pizza-pocket quente
    .desc = O recheio de queijo realmente cai bem quando quente.

ent-FoodDonkpocketHonk = honk-pocket
    .desc = O premiado donk-pocket que conquistou os corações de palhaços e humanos por igual.

ent-FoodDonkpocketHonkWarm = honk-pocket quente
    .desc = O premiado donk-pocket, agora quente e aconchegante.

ent-FoodDonkpocketBerry = berry-pocket
    .desc = Um donk-pocket irresistivelmente doce. Feito com 100% de sabor artificial.

ent-FoodDonkpocketBerryWarm = berry-pocket quente
    .desc = Um donk-pocket irresistivelmente doce, agora quente e delicioso.

ent-FoodDonkpocketStonk = stonk-pocket
    .desc = Saboroso, mas um triste lembrete do grande acidente de '24.

ent-FoodDonkpocketStonkWarm = stonk-pocket quente
    .desc = Saboroso, mas um triste lembrete do grande acidente de '24.

ent-FoodDonkpocketCarp = carp-pocket
    .desc = Uma edição perdida há muito tempo do donk-pocket, feita especificamente para salvadores trabalhadores.

ent-FoodDonkpocketCarpWarm = carp-pocket quente
    .desc = Uma edição perdida há muito tempo do donk-pocket, feita especificamente para salvadores trabalhadores. Agora bem quente.

ent-FoodDonkpocketDink = dink-pocket
    .desc = Um donk-pocket de marca não oficial, recheado com cenoura em conserva e envolto em algas. Melhor consumido frio, ou ainda melhor, não consumido de jeito nenhum.

ent-FoodDonkpocketMoth = moth-pocket
    .desc = Edição buzzy do donk-pocket criado durante protestos em massa ab-... Na verdade, quem se importa? Apenas donk-pockets para mariposas.

ent-FoodDonkpocketMothWarm = moth-pocket quente
    .desc = Edição buzzy do donk-pocket para mariposas, agora quentinho e ainda mais intrigante.
