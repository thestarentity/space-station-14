ent-EnergySword = espada de energia
    .desc = Uma espada muito barulhenta e perigosa com um feixe feito de plasma puro e altamente concentrado. Corta alvos desarmados como manteiga.

ent-EnergyDaggerLoud = espada de energia
    .desc = Uma faca não tão alta e perigosa, com um feixe feito de plasma puro e concentrado. Esta é completamente desprovida de disfarces.

ent-EnergyDagger = caneta
    .desc = Uma caneta de tinta escura.

ent-EnergyDaggerBox = caixa de faca de energia
    .desc = Uma pequena caixa contendo uma adaga eletroquímica. O embalagem se desintegra ao ser aberta, deixando nenhuma evidência para trás.

ent-EnergyCutlass = espada de energia
    .desc = Uma arma de energia exótica.

ent-EnergySwordDouble = espada de energia de dupla lâmina
    .desc = O estágio de comando do Sindicato pensou que ter uma única lâmina na espada de energia não era suficiente. Pode ser armazenado em bolsos.

ent-HyperEutacticBlade = espada hipereutática
    .desc = Uma arma supermassiva concebida para cortar o próprio tecido do espaço e do tempo ao meio, a lâmina hipereutática dinamicamente flash-forges uma estrutura nanocrística hipereutática capaz de passar através da maioria das formas conhecidas de matéria como uma faca quente cortando manteiga.

