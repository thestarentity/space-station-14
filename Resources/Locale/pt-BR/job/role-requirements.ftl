role-timer-department-insufficient = Você precisa de [color=yellow]{ $time }[/color] mais tempo de brincadeira no departamento [color={ $departmentColor }]{ $department }[/color] para desbloquear isso.
role-timer-department-too-high = Você precisa de [color=yellow]{ $time }[/color] menos tempo de brincadeira no departamento [color={ $departmentColor }]{ $department }[/color] para selecionar este. (Você está tentando jogar um papel de estagiário?)
...
role-timer-blacklisted-species = O seu personagem não pode ser uma das seguintes espécies para você selecionar isso:
role-timer-whitelisted-traits = Seu personagem deve ter uma das seguintes características para que você possa selecionar isso:
role-timer-blacklisted-traits = Seu personagem não pode ter nenhum dos seguintes traços para que você possa selecionar isso:
role-timer-locked = Bloqueado (passar o mouse para detalhes)
role-timer-department-unknown = Departamento Desconhecido
role-ban = Você foi banido desse cargo.
